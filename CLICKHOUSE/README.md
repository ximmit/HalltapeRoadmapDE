...
* **Array(Массив)** - хранит в себе N-ое кол-во элеметов любого типв данных. Надо запомнить что массивы не могут иметь разные типы данных, первый элемет начинается с 1.
...
Основная идея MergeTree состоит в том, что данные записываются на частям, небольшими кусками, а потом объединяются в фоновом режиме. Физически это выглядит следующим образом:

<p align="center">
    <img src="./../png/ch_merge.gif" alt="merge"/>
</p>

Имя файла у таких таблиц состоит из следующих частей:

<p align="center">
    <img src="./../png/ch_filename.jpg" alt="merge"/>
</p>
...
Да к сожалению ни материализованную (хоть она физически существует), ни алис мы вызвать через `SELECT *` не можем, чтобы их вызнать, необходимо в SELECT их имена, попробуйте это сделать сами на основе примера описанного выше.
...
<p align="center">
    <img src="./../png/ch_partition.jpg" alt="ch_partition"/>
</p>
...
Важно понимать что при разделении таблиц по патициям, аднные сливаются в рамках своих партиций
...
Напоминаю, что данный ключ создается с помощью ключевых конструкций `ORDER BY (...)` или `PRIMARY KEY (...)`(набор полей должен повторять ключ сортировки и не может быть тъбольше него). 
...
<p align="center">
    <img src="./../png/ch_minmax.png" alt="ch_minmax"/>
</p>
...
<p align="center">
    <img src="./../png/ch_set_idx.jpg" alt="ch_set_idx"/>
</p>